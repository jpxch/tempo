...
type WeeklyClientProps = {
  initialWeekItems: WeekItem[];
  initialProjects: Project[];
};

const DUE_OPTIONS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'This Weekend', 'Soon'];

export function WeeklyClient({ initialWeekItems, initialProjects }: WeeklyClientProps) {
  const router = useRouter();
  const [creating, setCreating] = useState(false);
  const [newTitle, setNewTitle] = useState('');
  const [newDueLabel, setNewDueLabel] = useState(DUE_OPTIONS[6]);
  ...
  async function handleCreate(event?: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await insertWeekItem({ title, dueLabel: newDueLabel, projectId: newProjectId });
      setCreating(false);
      setNewTitle('');
      setNewDueLabel(DUE_OPTIONS[6]);
      router.refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Could not create weekly item.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleUpdate(id: string, event?: React.FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await updateWeekItem(id, { title, dueLabel: editDueLabel });
      setEditingId(null);
      router.refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Could not update weekly item.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    ...
    setError(null);
    try {
      await deleteWeekItem(id);
      setPendingDeleteId(null);
      router.refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Could not delete weekly item.');
      setPendingDeleteId(null);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section className="space-y-6 comfort:space-y-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold comfort:text-4xl">Weekly Rhythm</h1>
      ...
      {creating && initialProjects.length === 0 && (
        <div className="rounded-3xl border border-amber-400/20 bg-amber-400/5 p-5 comfort:p-6">
          <p className="text-sm text-amber-300 comfort:text-base">
            You need a project before you can create a weekly item.{' '}
      ...
      {creating && initialProjects.length > 0 && (
        <div className="rounded-3xl border border-violet-400/30 bg-white/4 p-5 comfort:p-6">
          <form className="space-y-3 comfort:space-y-4" onSubmit={handleCreate}>
            <div>
              <label htmlFor="new-week-title" className="mb-1 block text-xs font-medium uppercase tracking-wider text-neutral-500 comfort:text-sm">
                What *
              </label>
              <input
                id="new-week-title"
            ...
            <div className="grid grid-cols-2 gap-3 comfort:gap-4">
              <div>
                <label htmlFor="new-week-due" className="mb-1 block text-xs font-medium uppercase tracking-wider text-neutral-500 comfort:text-sm">
                  When
                </label>
                <select
                  id="new-week-due"
              ...
              <div>
                <label htmlFor="new-week-project" className="mb-1 block text-xs font-medium uppercase tracking-wider text-neutral-500 comfort:text-sm">
                  Project *
                </label>
                <select
                  id="new-week-project"
      ...
      <div className="space-y-3 comfort:space-y-4">
        {initialWeekItems.length === 0 && !creating && (
        ...
        {initialWeekItems.map((item) => {
          const project = initialProjects.find((p) => p.id === item.projectId);
          ...
          return (
            ...
            >
              {editingId === item.id ? (
                <form className="space-y-3 comfort:space-y-4" onSubmit={(e) => handleUpdate(item.id, e)}>
                  <div>
                    <label htmlFor={`edit-title-${item.id}`} className="mb-1 block text-xs font-medium uppercase tracking-wider text-neutral-500 comfort:text-sm">
                      What *
                    </label>
                    <input
                  ...
                  <div>
                    <label htmlFor={`edit-due-${item.id}`} className="mb-1 block text-xs font-medium uppercase tracking-wider text-neutral-500 comfort:text-sm">
                      When
                    </label>
                    <select
                      id={`edit-due-${item.id}`}
                ...
                <div className="space-y-3">
                  <p className="text-sm text-neutral-300 comfort:text-base">
                    Delete this weekly item? This cannot be undone.
                  </p>
                  <div className="flex gap-3">
                    <button
                      type="button"
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="flex shrink-0 items-start gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => { setPendingDeleteId(item.id); setError(null); }}
